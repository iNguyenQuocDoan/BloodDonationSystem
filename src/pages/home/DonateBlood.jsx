...
const DonateBlood = () => {
  ...
  const fetchMyRegistrations = async () => {
    try {
      const response = await fetch(`/api/appointments/history?userId=${user.user_id}`);
  ...
  const handleRegister = async (slotId) => {
    ...
    if (myRegistrations && myRegistrations.length > 0) {
      const isRegistered = myRegistrations.some(reg => reg.Slot_ID === slotId);
      if (!isRegistered) {
        toast.info("Bạn chỉ được đăng ký hiến máu 1 lần trong 1 tháng kể từ lần đăng ký trước. Nếu muốn đổi ca, hãy liên hệ quản trị viên.");
        return;
      }
    }
  ...
  const canRegisterSlot = useCallback((slotDate) => {
    ...
    <div className="container mx-auto py-8 px-4">
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredSlots.map((slot) => {
            ...
            const isRegistered = myRegistrations && myRegistrations.some(reg => reg.Slot_ID === slot.Slot_ID);
            if (window && window.console) {
              console.log('[DEBUG][FE] Slot:', slot.Slot_ID, '| isRegistered:', isRegistered, '| myRegistrations:', myRegistrations);
              console.log('[DEBUG][FE] Slot:', slot.Slot_ID, '| Start_Time:', slot.Start_Time, '| End_Time:', slot.End_Time);
            }
            // Tìm ca đăng ký gần nhất
            const isLatestSlot = latestRegistration && slot.Slot_ID === latestRegistration.Slot_ID;
            // Ngày có thể đăng ký lại
            const canRegister = canRegisterSlot ? canRegisterSlot(slot.Slot_Date) : true;
                ...
                <div className="p-6">
                  ...
                  <button
                    ...
                    className={`w-full py-2 px-4 rounded transition duration-300 flex items-center justify-center font-semibold
                      ${isSlotFull ? 'bg-yellow-200 text-yellow-700 cursor-not-allowed' :
                        isRegistered ? 'bg-blue-200 text-blue-700 cursor-not-allowed' :
                        'bg-red-600 hover:bg-red-700 text-white'}`}
                    title={isRegistered ? 'Bạn đã đăng ký ca này.' :
                      isSlotFull ? 'Ca này đã đầy, vui lòng chọn ca khác.' : ''}
                  >
                    {isRegistered ? (
                      <>
                  ...
                  {!canRegister && isLatestSlot && nextRegisterDate && (
                    <div className="text-center text-blue-700 mt-2 text-sm">
                      Bạn có thể đăng ký lại sau ngày: <span className="font-semibold">{nextRegisterDate.toLocaleDateString('vi-VN')}</span>
                    </div>
                  )}
                </div>
              </div>
      ...
      {user && (
        <div className="mt-10 mb-8">
          <h2 className="text-2xl font-bold text-red-600 mb-4 text-center">Lịch sử đăng ký hiến máu của bạn</h2>
          {myRegistrations && myRegistrations.length === 0 ? (
            <div className="text-center text-gray-500">Bạn chưa đăng ký hiến máu nào.</div>
          ) : (
            <div className="overflow-x-auto">
              <table className="min-w-full bg-white rounded shadow">
                ...
                <tbody>
                  {myRegistrations && myRegistrations.map((reg) => (
                    <tr key={reg.Appointment_ID} className="border-b hover:bg-gray-50">
                      <td className="px-4 py-2">{formatDateVN(reg.Slot_Date)}</td>
                      <td className="px-4 py-2">{formatTimeVN(reg.Start_Time)} - {formatTimeVN(reg.End_Time)}</td>
                      <td className="px-4 py-2">
                        {reg.Status === 'P' && <span className="text-yellow-600 font-semibold">Chờ xác nhận</span>}
                        {reg.Status === 'A' && <span className="text-green-600 font-semibold">Được hiến</span>}
                        {reg.Status === 'R' && <span className="text-red-600 font-semibold">Từ chối</span>}
                      </td>
                      <td className="px-4 py-2">{reg.Status === 'R' && reg.Reject_Reason ? reg.Reject_Reason : '-'}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
